        ...
        <div className="flex items-center justify-between h-16">
          <div className="relative group">
            <button className="flex items-center space-x-2 bg-secondary text-white px-4 py-2 rounded-md hover:bg-secondary">
              <i class="fi fi-rr-apps flex items-center"></i>
              <span className='text-sm'>All Categories</span>
              <ChevronDown className="h-4 w-4" />
            </button>
            <div className="absolute z-50 hidden text-sm group-hover:block w-48 bg-white border rounded-md shadow-lg py-2 mt-1">
            ...
            {navItems.map((item) => (
              ...
              >
                ...
                {openDropdown === item.name && (
                  <div className="absolute z-50 w-48 bg-white border rounded-md shadow-lg py-2 ">
                    {item.items.map((subItem) => (
                      <Link
                        key={subItem.name}
                        to={subItem.link}
                        className="block px-4 py-2 text-sm text-gray-800 hover:text-secondary"
                      >
                        {subItem.name}
                      </Link>
                    ))}
