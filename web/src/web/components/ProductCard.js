...
const ProductCard = ({ product, viewType }) => {
  ...
  return (
    <div
      className={`bg-white rounded-lg border shadow-md shadow-green-200 border-gray-300 overflow-hidden relative group transform transition-all duration-300 hover:scale-105 ${
        viewType === "list" ? "flex" : ""
      }`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      ...
      >
        ...
        {isHovered && (
          <div className="absolute inset-0 bg-black bg-opacity-40 flex justify-center items-end pb-3 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="flex gap-4">
              <button
                onClick={() => handleCart(product)}
                className="p-3 bg-white rounded-full hover:bg-secondary hover:text-white transition-all"
              >
                <GiShoppingCart className="w-5 h-5" />
              </button>
              <button
                onClick={() => toast.error("This feature is not available right now")}
                className="p-3 bg-white rounded-full hover:bg-secondary hover:text-white transition-all"
              >
      ...
      >
        <span className="text-primary/70 capitalize text-xs font-semibold mb-1">
          {product.category_name}
        </span>

        <h5 className="text-sm font-semibold text-primary capitalize tracking-wide mb-1 hover:text-secondary">
          <Link
            className="hover:text-secondary"
            to={`/product/${product.product_id}`}
          >
